...
const API_BASE_URL = API_URL + 'api/v1/errorReport';

function* report(action) {
    try {
        const token = yield select(selectors.getAuthToken);
        const response = yield call(fetch, `${API_BASE_URL_USER}/`, {
            method: 'POST',
                body: JSON.stringify({...action.payload, user_id: selectors.getAuthUserID}),
                headers: {
